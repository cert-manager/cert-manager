...
![Before(Ingress)/After(Gateway) diag. App Devs: Ingress. Cluster Ops: Gateway](https://hackmd.io/_uploads/r1rgH2tblx.png)
...
1. **Single shared Gateway per cluster**: Mimics their current multi-tenant setup, but developers lose the ability to self-service their TLS configuration. The Gateway resource (which holds TLS config) must be cluster operator-controlled, forcing developers to file tickets for every hostname change—a massive step backward in velocity.
...
ListenerSet (introduced in [GEP-1713](https://gateway-api.sigs.k8s.io/geps/gep-1713/)) solves this by decoupling TLS configuration from the Gateway resource, allowing:
...
![Before: App devs managed Ingress. After: Cluster Ops manage Gateway, App Devs manage HTTPRoute](https://hackmd.io/_uploads/B1PRYpKZgl.png)
...
![Before(Ingress)/After(Gateway) diag. App Devs: Ingress. Cluster Ops: Gateway. Apps Devs: ListenerSet and HTTPRoute](https://hackmd.io/_uploads/B1PRYpKZgl.png)
