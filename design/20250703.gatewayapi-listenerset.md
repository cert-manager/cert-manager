...
- It is unclear whether the `X` and stable APIs can or should be used simultanously: can the user have an XGateway with HTTPRoute objects that point to it?
- Adding this feature would first require us to support Gateway API's `X` versions of the API (XListenerSet, XGateway, and XHTTPRoute).
- Using this feature would require the user to accept using the experimental APIs (XListenerSet, XGateway); migrating from Gateway to XGateway is easy, but the reverse might not be since `X` APIs might change at any point. Same for migrating back from XListenerSet to ListenerSet, which might even be more 
...
The first step is to add support for `X` resource in cert-manager through a new feature flag:
...
entry in `certificateRefs`. This limitation stems from the fact that
Gateway API's creators aim to avoid having to parse the certificates to be able
to route traffic.
