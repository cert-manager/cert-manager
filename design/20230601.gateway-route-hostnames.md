...
This reduces configuration duplication, and allows site owners to add hostnames without the involvement of the cluster owner, if permitted.
...
As with any configuration duplication, it is easy to forget updating in one place, causing difficult-to-find bugs, and requiring maintaining more documentation within a team.
...
Hostnames make more sense in Routes than in `Listener`s, as a single route may be used for both HTTP and HTTPS.
...
However, if some `Gateway` does not specify hostnames in `Listener`s, upgrading may cause certificates to be issued for hostnames not previously seen.
...
It will subscribe to `HTTPRoute`, `TLSRoute`, and similar resources, and not just `Gateway` resources.
