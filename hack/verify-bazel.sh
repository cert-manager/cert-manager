...
# By not setting up the go tool in the bazel circleci.dec.yaml environment at all, we still
# get a confusing error message, but we fail fast and it is clear that
# something is wrong:
#    gazelle: finding module path for import github.com/cert-manager/cert-manager/circleci.dec.yaml/unit/gen: exit status 1: build cache is required, but could not be located: GOCACHE is not defined and $HOME is not defined
