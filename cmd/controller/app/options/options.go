...
func AddConfigFlags(fs *pflag.FlagSet, c *config.ControllerConfiguration) {
	...
	fs.BoolVar(&c.EnableGatewayAPIXListenerSets, "enable-gateway-api-xlistenersets", c.EnableGatewayAPIXListenerSets, ""+
		"Whether XListenerSets support is enabled within cert-manager. The XListenerSet "+
		"feature gate must also be enabeld.")
...
func EnabledControllers(o *config.ControllerConfiguration) sets.Set[string] {
	...
	if utilfeature.DefaultFeatureGate.Enabled(feature.XListenerSets) && o.EnableGatewayAPI && o.EnableGatewayAPIXListenerSets {
