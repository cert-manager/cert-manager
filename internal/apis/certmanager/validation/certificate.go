...
func ValidateCertificateSpec(crt *internalcmapi.CertificateSpec, fldPath *field.Path) field.ErrorList {
	...
	if crt.RevisionHistoryLimit != nil && *crt.RevisionHistoryLimit < 0 {
		el = append(el, field.Invalid(fldPath.Child("revisionHistoryLimit"), *crt.RevisionHistoryLimit, "must not be less than 0"))
	}
