...
func validateCertificateRenewalWindows(window internalcmapi.CertificateRenewalWindows, fldPath *field.Path) field.ErrorList {
	...
	if window.CronSeconds == 0 {
		el = append(el, field.Required(fldPath.Child("cronSeconds"), "must be specified"))
	}

	if _, err := time.LoadLocation(window.Timezone); err != nil && window.Timezone != "" {
		el = append(el, field.Invalid(fldPath.Child("timezone"), "invalid value for timezone", "timezone must be IANA compliant"))
	}

	if _, err := utilpkg.CronParse(window.Cron, window.Timezone); err != nil {
		el = append(el, field.Invalid(fldPath.Child("cron"), fmt.Sprintf("invalid cron syntax: %s", err.Error()), "cron needs to follow: cron.Minute | cron.Hour | cron.Dom | cron.Month | cron.Dow"))
	}
