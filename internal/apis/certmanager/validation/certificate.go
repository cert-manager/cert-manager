...
var keyAlgToAllowedSigAlgs = map[internalcmapi.PrivateKeyAlgorithm][]internalcmapi.SignatureAlgorithm{
	...
	internalcmapi.MLDSA44KeyAlgorithm: {
		internalcmapi.PureMLDSA44,
	},
...
func validateCertificateSpec(crt, oldCrt *internalcmapi.CertificateSpec, fldPath *field.Path) field.ErrorList {
	...
	if crt.PrivateKey != nil {
		...
		case internalcmapi.Ed25519KeyAlgorithm:
			break
		case internalcmapi.MLDSA44KeyAlgorithm:
			break
		case internalcmapi.MLDSA65KeyAlgorithm:
			break
		default:
			el = append(el, field.Invalid(fldPath.Child("privateKey", "algorithm"), crt.PrivateKey.Algorithm, "must be either empty or one of rsa, ecdsa, ed25519, mldsa44, or mldsa65"))
		}
	}
