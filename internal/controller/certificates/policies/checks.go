...
func currentSecretValidForSpec(input Input) *Violation[MaybeReason[IssuanceReason]] {
	...
	// nolint: staticcheck // Ignore deprecation warning for FuzzyX509AltNamesMatchSpec, as it is used here for backwards compatibility.
	violations, err := pki.FuzzyX509AltNamesMatchSpec(x509Cert, input.Certificate.Spec)
	if err != nil {
...
// Certificate's SecretTemplate. It only compares the values of the annotations.
// It does not check the presence or absence of annotations or labels.
func SecretSecretTemplateMismatch(input Input) *Violation[MaybeReason[PostIssuanceReason]] {
	...
	annotations := maps.Keys(input.Certificate.Spec.SecretTemplate.Annotations)
	slices.Sort(annotations)
	for _, kSpec := range annotations {
		vSpec := input.Certificate.Spec.SecretTemplate.Annotations[kSpec]
		v, ok := input.Secret.Annotations[kSpec]
		if !ok || v == vSpec {
			continue
		}

		return NewPostIssuanceViolation(SecretMetadataMismatch, fmt.Sprintf("Secret annotation %q has value %q, expected %q", kSpec, v, vSpec))
	}

	labels := maps.Keys(input.Certificate.Spec.SecretTemplate.Labels)
	slices.Sort(labels)
	for _, kSpec := range labels {
		vSpec := input.Certificate.Spec.SecretTemplate.Labels[kSpec]
		v, ok := input.Secret.Labels[kSpec]
		if !ok || v == vSpec {
			continue
		}

		return NewPostIssuanceViolation(SecretMetadataMismatch, fmt.Sprintf("Secret label %q has value %q, expected %q", kSpec, v, vSpec))
	}
...
func SecretCertificateDetailsAnnotationsMismatch(input Input) *Violation[MaybeReason[PostIssuanceReason]] {
	...
	annotations := maps.Keys(dataAnnotations)
	slices.Sort(annotations)
	for _, kSpec := range annotations {
		vSpec := dataAnnotations[kSpec]
		v, ok := input.Secret.Annotations[kSpec]
		if !ok || v == vSpec {
			continue
		}
