...
type ACMEIssuer struct {
	...
	// +optional
	AccountPrivateKey *AccountPrivateKey `json:"accountPrivateKey,omitempty"`
...
// +kubebuilder:validation:XValidation:rule="!has(self.algorithm) || self.algorithm == 'RSA' ? (!has(self.size) || (self.size >= 2048 && self.size <= 8192)) : (!has(self.size) || self.size in [256, 384, 521])",message="size must be between 2048 and 8192 for RSA, or one of 256, 384 or 521 for ECDSA"
type AccountPrivateKey struct {
	...
	// +optional
	Algorithm AccountKeyAlgorithm `json:"algorithm,omitempty"`
