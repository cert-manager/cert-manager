...
func TestSyncSolverPickerPath(t *testing.T) {
	...
	accessDeniedError := &types.AccessDeniedException{}
	...
	for k, test := range scenarios {
		t.Run(k, func(t *testing.T) {
			...
			retrySolverIml := newRetrySolver(&dummySolver, fakeOrder{[]*cmacme.Order{baseOrder}}, c.helper)
			c.dnsSolver = retrySolverIml

			test.builder.Start()

			c.Sync(t.Context(), test.challenge)

			test.builder.CheckAndFinish(nil)
		})
	}
}
