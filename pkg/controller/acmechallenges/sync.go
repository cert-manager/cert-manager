...
func (c *controller) Sync(ctx context.Context, chOriginal *cmacme.Challenge) (err error) {
	...
	// Backfill PresentedAt for already-presented challenges when wait-based
	// self-check bypass is configured. This covers upgrade or pre-existing-
	// object cases where Presented was recorded before the controller started
	// writing PresentedAt.
