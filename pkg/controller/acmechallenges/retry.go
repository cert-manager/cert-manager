...
func (r *retrySolver) Present(ctx context.Context, issuer cmapi.GenericIssuer, ch *cmacme.Challenge) error {
	...
	if err != nil && r.needsRetry(err) {
		return r.retry(ctx, ch, issuer, err, func(newCtx context.Context, newIssuer cmapi.GenericIssuer, newCh *cmacme.Challenge) error {
			return r.Present(newCtx, newIssuer, newCh)
		})
	}
	return err
}
...
func (r *retrySolver) Check(ctx context.Context, issuer cmapi.GenericIssuer, ch *cmacme.Challenge) error {
	...
	if err != nil && r.needsRetry(err) {
		return r.retry(ctx, ch, issuer, err, func(newCtx context.Context, newIssuer cmapi.GenericIssuer, newCh *cmacme.Challenge) error {
			return r.Check(newCtx, newIssuer, newCh)
		})
	}
	return err
}
...
func (r *retrySolver) CleanUp(ctx context.Context, ch *cmacme.Challenge) error {
	...
	if err != nil && r.needsRetry(err) {
		return r.retry(ctx, ch, nil, err, func(newCtx context.Context, _ cmapi.GenericIssuer, newCh *cmacme.Challenge) error {
			return r.CleanUp(newCtx, newCh)
		})
	}
	return err
}
...
func (r *retrySolver) retry(
	...
	if retryErr != nil {
		log.V(logf.InfoLevel).Info("retry request failed", "challenge", ch.Name, "err", retryErr)
		return err
