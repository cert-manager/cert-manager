...
func TestIssuer(t *testing.T) {
	var nilIssuer *cmapi.Issuer
	var nilClusterIssuer *cmapi.ClusterIssuer
	type testT struct {
		Name                   string
		Kind                   string
		Namespace              string
	...
	for _, row := range tests {
		t.Run(row.Name, func(t *testing.T) {
			...
			actual, err := c.Issuer(cmmeta.ObjectReference{Name: row.Name, Kind: row.Kind}, row.Namespace)
			if err != nil && !row.Err {
