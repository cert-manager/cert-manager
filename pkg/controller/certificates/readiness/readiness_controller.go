...
func (c *controller) computeNextCheck(now time.Time, retryAfter time.Duration) time.Time {
	...
	jit := time.Duration(rand.Int63n(int64(2*ariJitterPct*
		float64(d)))) - time.Duration(ariJitterPct*float64(d))
...
func (c *controller) useARIForRenewal(ctx context.Context, crt *cmapi.Certificate, x509cert *x509.Certificate, key types.NamespacedName) *metav1.Time {
	...
	ariInfo, err := c.getARIInfo(ctx, crt, x509cert)
	...
	if ariStatus.NextCheck != nil {
		c.scheduledWorkQueue.Add(key, ariStatus.NextCheck.Time.Sub(now))
	}

	return renewalTime
}

func (c *controller) getARIInfo(ctx context.Context, cmcrt *cmapi.Certificate, crt *x509.Certificate) (*acmeapi.RenewalInfoResponse, error) {
	genericIssuer, err := c.helper.GetGenericIssuer(cmcrt.Spec.IssuerRef, cmcrt.Namespace)
	if err != nil {
		return nil, err
	}
