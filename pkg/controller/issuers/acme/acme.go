...
type ACME struct {
	...
	secretsClient coreclient.SecretsGetter
...
func (a *ACME) Setup(ctx context.Context, issuer cmapi.GenericIssuer) error {
	...
	case issuer.GetSpec().ACME.DisableAccountKeyGeneration && apierrors.IsNotFound(err):
		...
		apiutil.SetIssuerCondition(issuer, cmapi.IssuerConditionReady, cmmeta.ConditionFalse, errorAccountVerificationFailed, wrapErr.Error())
		return wrapErr
	...
	if eabObj := issuer.GetSpec().ACME.ExternalAccountBinding; eabObj != nil {
		eabKey, err := a.getEABKey(issuer, resourceNamespace)
		switch {
...
func (a *ACME) registerAccount(ctx context.Context, issuer cmapi.GenericIssuer,
	cl client.Interface, eabAccount *acmeapi.ExternalAccountBinding) (*acmeapi.Account, error) {
	emailurl := []string(nil)
...
func (a *ACME) getEABKey(issuer cmapi.GenericIssuer, ns string) ([]byte, error) {
...
func (a *ACME) Implements(issuer cmapi.GenericIssuer) bool {
	return issuer.GetSpec().ACME != nil
}

func (a *ACME) ReferencesSecret(issuer cmapi.GenericIssuer, secret *corev1.Secret) bool {
	acmeSpec := issuer.GetSpec().ACME
	if acmeSpec == nil {
		return false
	}

	if a.issuerOptions.ResourceNamespace(issuer) != secret.Namespace {
		return false
	}

	if acmeSpec.PrivateKey.Name == secret.Name {
		return true
	}

	if acmeSpec.ExternalAccountBinding != nil && acmeSpec.ExternalAccountBinding.Key.Name == secret.Name {
		return true
	}
