	...
	tests := map[string]testSetupT{
		"if client builder fails then should error": {
			...
			expectedCondition: &cmapi.IssuerCondition{
				Reason:  "ErrorClient",
				Message: "Failed to build Venafi client: this is an error",
				Status:  "False",
			},
			expectedEvents: []string{
				"Warning ErrorClient this is an error",
			},
		},
		...
		"if ping fails then should error": {
			...
			expectedCondition: &cmapi.IssuerCondition{
				Reason:  "ErrorPing",
				Message: "Failed to connect to Venafi endpoint: this is a ping error",
				Status:  "False",
			},
			expectedEvents: []string{
				"Warning ErrorPing this is a ping error",
			},
		},
