...
// Venafi Issuer controller can process Issuers when using all its default
...
func TestControllerIntegration(t *testing.T) {
	...
	err = c.ProcessItem(ctx, "ns1/issuer-1")
	assert.Error(t, err)

	issuer, err := builder.FakeCMClient().CertmanagerV1().Issuers("ns1").Get(ctx, "issuer-1", metav1.GetOptions{})
