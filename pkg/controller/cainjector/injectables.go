...
func (t *mutatingWebhookTarget) SetCA(data []byte) {
	for ind := range t.obj.Webhooks {
		if utilfeature.DefaultFeatureGate.Enabled(feature.CAInjectorMerging) {
			...
			// fallback
			bundle, err := pki.AppendCertificatesToBundle(t.obj.Webhooks[ind].ClientConfig.CABundle, data)
			if err != nil {
				bundle = data
			}
...
func (t *apiServiceTarget) SetCA(data []byte) {
	if utilfeature.DefaultFeatureGate.Enabled(feature.CAInjectorMerging) {
		...
		// This mirrors the old behavior of this function so is a reasonable
		// fallback
		bundle, err := pki.AppendCertificatesToBundle(t.obj.Spec.CABundle, data)
		if err != nil {
			bundle = data
		}
...
func (t *crdConversionTarget) SetCA(data []byte) {
	...
	if utilfeature.DefaultFeatureGate.Enabled(feature.CAInjectorMerging) {
		...
		// This mirrors the old behavior of this function so is a reasonable
		// fallback
		bundle, err := pki.AppendCertificatesToBundle(t.obj.Spec.Conversion.Webhook.ClientConfig.CABundle, data)
		if err != nil {
			bundle = data
		}
