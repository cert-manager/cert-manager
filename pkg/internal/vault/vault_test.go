...
func TestExtractCertificatesFromVaultCertificateSecret(t *testing.T) {
	tests := map[string]testExtractCertificatesFromVaultCertT{
		...
		"when a Vault engine is an intermediate CA and has duplicates of intermediate, and its parent is a intermediate CA": {
	...
	for name, test := range tests {
		...
		if test.expectedCert != string(cert) {
			t.Errorf("%s: unexpected leaf certificate, exp=%s, got=%s", name, test.expectedCert, cert)
		}
		if test.expectedCA != string(ca) {
