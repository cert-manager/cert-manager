...
func (v *Venafi) RequestCertificate(csrPEM []byte, duration time.Duration, customFields []api.CustomField) (string, error) {
	...
	// If the connector is Control Plane, Self-Hosted, we unconditionally reset any prior failed enrollment
	...
	// race in Control Plane, Self-Hosted.
	//
	...
	// large Control Plane, Self-Hosted instances.
	//
