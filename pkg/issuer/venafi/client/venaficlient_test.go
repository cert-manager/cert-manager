...
func TestConfigForIssuerT(t *testing.T) {
	...
	tests := map[string]testConfigForIssuerT{
		"if Certificate Manager spec has no options in config then should error": {
			iss:         baseIssuer,
			CheckFn:     checkNoConfigReturned,
			expectedErr: true,
		},
		"if Certificate Manager, Self-Hosted but getting secret fails, should error": {
			iss:           tppIssuer,
			secretsLister: generateSecretLister(nil, errors.New("this is a network error")),
			CheckFn:       checkNoConfigReturned,
			expectedErr:   true,
		},
		"if Certificate Manager, Self-Hosted and neither caBundle nor caBundleSecretRef is specified, CA bundle is not set in vcert config": {
		...
		"if Certificate Manager, Self-Hosted and secret returns user/pass, should return config with those credentials": {
			iss: tppIssuer,
			secretsLister: generateSecretLister(&corev1.Secret{
				Data: map[string][]byte{
					tppUsernameKey: []byte(username),
		...
		"if Certificate Manager, Self-Hosted and secret returns user/pass/clientId, should return config with those credentials": {
			iss: tppIssuer,
			secretsLister: generateSecretLister(&corev1.Secret{
				Data: map[string][]byte{
					tppUsernameKey: []byte(username),
		...
		"if Certificate Manager, Self-Hosted and secret returns access-token, should return config with those credentials": {
			iss: tppIssuer,
			secretsLister: generateSecretLister(&corev1.Secret{
				Data: map[string][]byte{
		...
		// NOTE: Below scenarios assume valid Certificate Manager, Self-Hosted CAs, the scenarios with invalid Certificate Manager, Self-Hosted CAs are run part of TestCaBundleForVcertTPP test
		"if Certificate Manager, Self-Hosted and a good caBundle specified, CA bundle should be added to ConnectionTrust and Client in vcert config": {
		...
		"if Certificate Manager, Self-Hosted and a good caBundleSecretRef specified, CA bundle should be added to ConnectionTrust and Client in vcert config": {
		...
		"if Certificate Manager, SaaS but getting secret fails, should error": {
			iss:           cloudIssuer,
			secretsLister: generateSecretLister(nil, errors.New("this is a network error")),
			CheckFn:       checkNoConfigReturned,
			expectedErr:   true,
		},
		"if Certificate Manager, SaaS and secret but no secret key ref, should use API key at default index": {
			iss: cloudIssuer,
			secretsLister: generateSecretLister(&corev1.Secret{
				Data: map[string][]byte{
		...
		"if Certificate Manager, SaaS and secret with secret key ref, should use API key at default index": {
		...
		"if Certificate Manager, Self-Hosted and SaaS, should chose Certificate Manager, Self-Hosted": {
...
func TestCaBundleForVcertTPP(t *testing.T) {
	...
	tests := map[string]testConfigForIssuerT{
		"if Certificate Manager, Self-Hosted and neither of caBundle nor caBundleSecretRef is specified, CA bundle is not returned": {
			iss:         tppIssuer,
			caBundle:    "",
			expectedErr: false,
		},
		"if Certificate Manager, Self-Hosted and caBundle is specified, correct CA bundle from CABundle should be returned": {
			iss:         tppIssuerWithCABundle,
			caBundle:    testLeafCertificate,
			expectedErr: false,
		},
		"if Certificate Manager, Self-Hosted and caBundleSecretRef is specified, correct CA bundle from CABundleSecretRef should be returned": {
			iss:      tppIssuerWithCABundleSecretRef,
			caBundle: testLeafCertificate,
			secretsLister: generateSecretLister(&corev1.Secret{
				Data: map[string][]byte{
		...
		"if Certificate Manager, Self-Hosted and caBundleSecretRef is specified without `key`, correct CA bundle from CABundleSecretRef with default key should be returned": {
		...
		"if Certificate Manager, Self-Hosted and caBundleSecretRef is specified, but getting secret fails should error": {
			iss:           tppIssuerWithCABundleSecretRef,
			caBundle:      testLeafCertificate,
			secretsLister: generateSecretLister(nil, errors.New("this is a network error")),
			expectedErr:   true,
		...
		// "if Certificate Manager, Self-Hosted and caBundle is specified, a bad bundle from CABundle should error"
		// "if Certificate Manager, Self-Hosted and caBundleSecretRef is specified, a bad bundle from a CABundleSecretRef should error"
	}
