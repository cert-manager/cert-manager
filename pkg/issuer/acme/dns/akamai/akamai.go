...
func (a *DNSProvider) Present(domain, fqdn, zone, value string) error {
	return a.setTxtRecord(fqdn, zone, &dns01Record{value, 60})
}

// CleanUp removes the TXT record matching the specified parameters
func (a *DNSProvider) CleanUp(domain, fqdn, zone, value string) error {
	return a.setTxtRecord(fqdn, zone, nil)
}
...
func (a *DNSProvider) setTxtRecord(fqdn string, zone string, dns01Record *dns01Record) error {
	zoneData, err := a.loadZoneData(zone)
	if err != nil {
		return errors.Wrapf(err, "failed to load zone data for %q", zone)
	}

	recordName, err := makeTxtRecordName(fqdn, zone)
	if err != nil {
	...
	if updated, err := zoneData.setTxtRecord(recordName, dns01Record); !updated || err != nil {
		if err != nil {
			return errors.Wrapf(err, "failed to set TXT record in %q", zone)
		}

		return errors.Errorf("no %q TXT record found in %q", recordName, zone)
	}

	newSerial, err := zoneData.incSoaSerial()
	if err != nil {
		return errors.Wrapf(err, "failed to increment SOA serial for %q", zone)
	}

	if err := a.saveZoneData(zone, zoneData); err != nil {
		return errors.Wrapf(err, "failed to save zone data for %q", zone)
	}

	klog.V(4).Infof("Updated Akamai TXT record for %q on %q using SOA serial of %d", recordName, zone, newSerial)
