...
func (a *DNSProvider) Present(ctx context.Context, domain, fqdn, value string) error {
	...
	logf.FromContext(ctx).V(logf.DebugLevel).Info("Calculated hosted domain", "hosted_domain", hostedDomain)

	recordName, err := makeTxtRecordName(fqdn, hostedDomain)
	if err != nil {
		return fmt.Errorf("edgedns: failed to create TXT record name: %w", err)
	}
	logf.FromContext(ctx).V(logf.DebugLevel).Info("Calculated TXT record name", "record_name", recordName)
...
func (a *DNSProvider) CleanUp(ctx context.Context, domain, fqdn, value string) error {
	...
	if len(newRData) > 0 {
		...
		logf.FromContext(ctx).V(logf.DebugLevel).Info("updating Akamai TXT record", "record_name", existingRec.Name, "data", newRData)
	...
	logf.FromContext(ctx).V(logf.DebugLevel).Info("deleting Akamai TXT record", "record_name", existingRec.Name)
