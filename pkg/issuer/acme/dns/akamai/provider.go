...
func (s *Solver) loadConfig(cfgJSON extapi.JSON) (*cmapi.ACMEIssuerDNS01ProviderAkamai, error) {
	cfg := cmapi.ACMEIssuerDNS01ProviderAkamai{}
...
func (s *Solver) buildDNSProvider(ch *cmapi.ChallengeRequest) (*DNSProvider, error) {
	...
	accessToken, err := loadSecretKeySelector(l, cfg.AccessToken, "")
	if err != nil {
		return nil, err
	}
	clientSecret, err := loadSecretKeySelector(l, cfg.ClientSecret, "")
	if err != nil {
		return nil, err
	}
	clientToken, err := loadSecretKeySelector(l, cfg.ClientToken, "")
	if err != nil {
		return nil, err
	}

	p, err := NewDNSProvider(cfg.ServiceConsumerDomain, string(clientToken), string(clientSecret), string(accessToken))
	if err != nil {
