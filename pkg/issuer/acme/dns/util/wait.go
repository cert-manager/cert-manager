...
func lookupNameservers(ctx context.Context, fqdn string, nameservers []string) ([]string, error) {
	...
	logf.FromContext(ctx).V(logf.DebugLevel).Info("Searching fqdn", "fqdn", fqdn, "seed_nameservers", nameservers)
	...
	if len(authoritativeNss) > 0 {
		logf.FromContext(ctx).V(logf.DebugLevel).Info("Returning authoritative nameservers", "authoritative_nameservers", authoritativeNss)
...
func FindZoneByFqdn(ctx context.Context, fqdn string, nameservers []string) (string, error) {
	...
	if zone, ok := fqdnToZone[fqdn]; ok {
		...
		logf.FromContext(ctx).V(logf.DebugLevel).Info("Returning cached zone record", "zone_record", zone, "fqdn", fqdn)
		return zone, nil
	}
	fqdnToZoneLock.RUnlock()
	...
	for _, index := range labelIndexes {
		...
		for _, ans := range in.Answer {
			if soa, ok := ans.(*dns.SOA); ok {
				...
				logf.FromContext(ctx).V(logf.DebugLevel).Info("Returning discovered zone record", "zone_record", zone, "fqdn", fqdn)
				return zone, nil
			}
		}
	}
