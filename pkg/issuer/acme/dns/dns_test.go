...
func TestPresent(t *testing.T) {
	tests := map[string]struct {
		...
		"builds a basic challenge request": {
			providers: []v1alpha1.ACMEIssuerDNS01Provider{
				{Name: "test", CNAMEStrategy: v1alpha1.NoneStrategy, Webhook: &v1alpha1.ACMEIssuerDNS01ProviderWebhook{}},
			},
			challenge: v1alpha1.Challenge{
	...
	for n, test := range tests {
		t.Run(n, func(t *testing.T) {
			s := &Solver{
				providers: test.solvers,
			}

			issuer := newIssuer("testissuer", "default", test.providers)

			err := s.Present(context.TODO(), issuer, &test.challenge)
