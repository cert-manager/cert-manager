...
func (s *Solver) loadConfig(cfgJSON extapi.JSON) (*cmapi.ACMEIssuerDNS01ProviderRoute53, error) {
	cfg := cmapi.ACMEIssuerDNS01ProviderRoute53{}
...
func (s *Solver) buildDNSProvider(ch *cmapi.ChallengeRequest) (*DNSProvider, error) {
	...
	secretAccessKey, err := loadSecretKeySelector(l, cfg.SecretAccessKey, "")
	if err != nil {
		return nil, err
	}

	if cfg.AccessKeyID == "" && len(secretAccessKey) == 0 {
		...
		return NewDNSProvider("", "", cfg.HostedZoneID, "", true)
	}

	if cfg.AccessKeyID == "" || len(secretAccessKey) == 0 {
		// It's always an error to set one of those but not the other
		return nil, fmt.Errorf("unable to construct route53 provider: both access key and secret key must be provided")
	}

	return NewDNSProvider(cfg.AccessKeyID, string(secretAccessKey), cfg.HostedZoneID, cfg.Region, false)
}
