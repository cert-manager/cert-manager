...
func (d *sessionProvider) GetSession(ctx context.Context) (aws.Config, error) {
	...
	if d.Role != "" && d.WebIdentityToken == "" {
		...
		if err != nil {
			return aws.Config{}, fmt.Errorf("unable to assume role: %w", wrapInRedactedError(err))
		}
	...
	if d.Role != "" && d.WebIdentityToken != "" {
		...
		if err != nil {
			return aws.Config{}, fmt.Errorf("unable to assume role with web identity: %w", wrapInRedactedError(err))
		}
...
func (r *DNSProvider) changeRecord(ctx context.Context, action route53types.ChangeAction, fqdn, value string, ttl int) error {
	...
	if err != nil {
		...
		return fmt.Errorf("failed to change Route 53 record set: %w", wrapInRedactedError(err))
	...
	return util.WaitFor(120*time.Second, 4*time.Second, func() (bool, error) {
		...
		if err != nil {
			return false, fmt.Errorf("failed to query Route 53 change status: %w", wrapInRedactedError(err))
		}
...
func (r *DNSProvider) getHostedZoneID(ctx context.Context, fqdn string) (string, error) {
	...
	if err != nil {
		return "", wrapInRedactedError(err)
	}
...
func wrapInRedactedError(err error) error {
	if err == nil {
		return nil
	}
