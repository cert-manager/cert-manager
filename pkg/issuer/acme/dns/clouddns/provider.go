...
func (s *Solver) loadConfig(cfgJSON extapi.JSON) (*cmapi.ACMEIssuerDNS01ProviderCloudDNS, error) {
	cfg := cmapi.ACMEIssuerDNS01ProviderCloudDNS{}
...
func (s *Solver) buildDNSProvider(ch *cmapi.ChallengeRequest) (*DNSProvider, error) {
	...
	if cfg.ServiceAccount.Name == "" {
		...
		p, err := NewDNSProviderAmbientCredentials(cfg.Project)
		if err != nil {
			return nil, err
		}
	...
	serviceAccount, err := loadSecretKeySelector(l, cfg.ServiceAccount, "")
	if err != nil {
		return nil, err
	}

	return NewDNSProviderServiceAccountBytes(cfg.Project, serviceAccount)
}
