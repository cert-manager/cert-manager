...
func (s *Solver) loadConfig(cfgJSON extapi.JSON) (*cmapi.ACMEIssuerDNS01ProviderDigitalOcean, error) {
	cfg := cmapi.ACMEIssuerDNS01ProviderDigitalOcean{}
...
func (s *Solver) buildDNSProvider(ch *cmapi.ChallengeRequest) (*DNSProvider, error) {
	...
	token, err := loadSecretKeySelector(l, cfg.Token, "")
	if err != nil {
		return nil, err
	}

	return NewDNSProviderCredentials(string(token))
}
