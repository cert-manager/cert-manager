...
func applyRenewBeforeWithWindows(notAfter, notBefore, desiredRenewalTime time.Time, windows []apiv1.CertificateRenewalWindows) RenewalResult {
	...
	for _, w := range windows {
		...
		windowDuration := time.Duration(w.CronSeconds) * time.Second

		// Any renewal logic should only start after notBefore and before notAfter. Bounds are [notBefore - dur, notAfter + dur)
		searchMin := notBefore.Add(-windowDuration)
		searchMax := notAfter.Add(windowDuration)

		bestBefore = findBeforeDesired(cronSched, searchMin, searchMax, desiredRenewalTime, bestBefore, windowDuration)
		bestAfter = findAfterDesired(cronSched, searchMin, searchMax, desiredRenewalTime, bestAfter, windowDuration)
	}
...
func bsFindEarliestWindowBeforeDesired(sched util.CronSchedule, minTime, maxTime time.Time) *time.Time {
