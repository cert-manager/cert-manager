...
func RenewalTime(notBefore, notAfter time.Time, renewBefore *metav1.Duration, renewBeforePercentage *int32, renewalSpec *apiv1.CertificateRenewal) RenewalResult {
	...
	fmt.Printf("desired renewal time %s\n", rt.String())
	...
	for _, w := range windows {
		...
		if w.Timezone != "" {
			if tz, err := time.LoadLocation(w.Timezone); err == nil {
				...
				return RenewalResult{
					FinalRenewalTime: nil,
					NonWindowError:   fmt.Errorf("error parsing cron in window %s", err.Error()),
				}
			}
		}
		...
		bestBefore = findBeforeDesired(cronSched, searchMin, desiredRenewalTime, bestBefore, w.Duration.Duration)
		bestAfter = findAfterDesired(cronSched, searchMax, desiredRenewalTime, bestAfter, w.Duration.Duration)
	}
...
func findBeforeDesired(cronSched cron.Schedule, searchMin, desiredRenewalTime time.Time, bestBefore *time.Time, duration time.Duration) *time.Time {
	lastStart := bsFindEarliestWindowBeforeDesired(cronSched, searchMin, desiredRenewalTime)
	fmt.Printf("last Start time %s", lastStart.String())
	if lastStart != nil {
	...
	fmt.Printf("bestBefore Start time %s", bestBefore.String())
	return bestBefore
}

func findAfterDesired(cronSched cron.Schedule, searchMax, desiredRenewalTime time.Time, bestAfter *time.Time, duration time.Duration) *time.Time {
