...
func RenewalTime(notBefore, notAfter time.Time, renewBefore *metav1.Duration, renewBeforePercentage *int32, renewalSpec *apiv1.CertificateRenewal, opts ...RenewalTimeOptions) (*metav1.Time, error) {
	...
	if o.ariInfo != nil && !o.ariInfo.SuggestedWindow.Start.IsZero() && !o.ariInfo.SuggestedWindow.End.IsZero() {
		rt = metav1.NewTime(selectRandTimeInARIWindow(o.ariInfo.SuggestedWindow.Start, o.ariInfo.SuggestedWindow.End).Truncate(time.Second))
	} else {
...
func desiredRenewalTime(actualDuration time.Duration, renewBefore *metav1.Duration, renewBeforePercentage *int32) time.Duration {
	...
	if renewBefore != nil && renewBefore.Duration > 0 && renewBefore.Duration < actualDuration {
		...
		return RenewBeforeFromPercentage(actualDuration, *renewBeforePercentage)
	}
...
// RenewBeforeFromPercentage calculates duration * percentage / 100 without
// overflowing int64 nanoseconds for long certificate durations.
func RenewBeforeFromPercentage(duration time.Duration, percentage int32) time.Duration {
	return duration/100*time.Duration(percentage) + duration%100*time.Duration(percentage)/100
}
	...
	for _, w := range windows {
		...
		if w.Timezone != "" {
			if tz, err := time.LoadLocation(w.Timezone); err == nil {
				...
				return &metav1.Time{Time: desiredRenewalTime}, fmt.Errorf("error parsing timezone in window: %w", err)
			}
		}

		cronSched, err := util.CronParse(w.Cron, loc.String())
		if err != nil {
			return &metav1.Time{Time: desiredRenewalTime}, fmt.Errorf("error parsing cron in window: %w", err)
		}
