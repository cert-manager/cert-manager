...
import (
	...
	"github.com/cert-manager/cert-manager/pkg/util/pki/internal/infohash"
)
...
func CertificateInfoHash(cert *cmapi.Certificate) (string, error) {
	hashBytes, err := CertificateHasher(cert).Sum()
	if err != nil {
		return "", err
	}

	return base32.HexEncoding.EncodeToString(hashBytes), nil
}

var ErrInvalidHash = infohash.ErrInvalidHash

type FieldChangedError = infohash.FieldChangedError

func IsCertificateUpToDateWithInfoHash(cert *cmapi.Certificate, hashString string) error {
	bytes, err := base32.HexEncoding.DecodeString(hashString)
	if err != nil {
		return err
	}
