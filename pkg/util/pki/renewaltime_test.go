...
func mustLocDenver(t *testing.T) *time.Location {
	t.Helper()
...
func TestRenewalWithWindowsForRenewBefore(t *testing.T) {
	...
	denverMDT := midnightUTC(time.Date(2026, time.March, 8, 0, 0, 0, 0, time.UTC))    // Approximate daylight savings start time. This is intentionally set to 2026 to avoid future flaky tests.
	...
	tests := map[string]scenario{
		...
		"single window, renewal time within window, different timezone": {
			notAfter:            future.Add(48 * time.Hour),
			targetRenewalTime:   time.Date(future.Year(), future.Month(), future.Day(), 10, 30, 0, 0, mustLocDenver(t)),
			expectedRenewalTime: time.Date(future.Year(), future.Month(), future.Day()+1, 0, 0, 0, 0, mustLocDenver(t)).Add(-(13*time.Hour + 30*time.Minute)).UTC(),
			notBefore:           midnightUTC(now.AddDate(0, 0, -10)),
			renewalSpec: &apiv1.CertificateRenewal{
				Policy: apiv1.CertificateRenewalPolicyRenewBefore,
				Windows: []apiv1.CertificateRenewalWindows{
		...
		"single window, renewal time in daylight savings (on to off) time zone": {
			...
			targetRenewalTime:   time.Date(denverMST.Year(), denverMST.Month(), denverMST.Day()-1, 15, 30, 0, 0, mustLocDenver(t)),
			expectedRenewalTime: time.Date(denverMST.Year(), denverMST.Month(), denverMST.Day()-1, 0, 0, 0, 0, mustLocDenver(t)).Add((35*time.Hour + 0*time.Minute)).UTC(),
...
func TestRenewalWithDisable(t *testing.T) {
	...
	res := RenewalTime(now, notAfter, &metav1.Duration{Duration: 20 * time.Hour}, nil, &apiv1.CertificateRenewal{
		Policy: apiv1.CertificateRenewalPolicyDisabled,
	})

	assert.Nil(t, res.WindowError)
	assert.Nil(t, res.FinalRenewalTime)
}
