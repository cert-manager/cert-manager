...
func IPAddressesForCertificate(crt *cmapi.Certificate) []net.IP {
...
func URIsForCertificate(crt *cmapi.Certificate) ([]*url.URL, error) {
...
func DNSNamesForCertificate(crt *cmapi.Certificate) ([]string, error) {
...
func OrganizationForCertificate(crt *cmapi.Certificate) []string {
...
func SubjectForCertificate(crt *cmapi.Certificate) cmapi.X509Subject {
	if crt.Spec.Subject == nil {
		return cmapi.X509Subject{}
	}
...
func BuildKeyUsages(usages []cmapi.KeyUsage, isCA bool) (ku x509.KeyUsage, eku []x509.ExtKeyUsage, err error) {
	var unk []cmapi.KeyUsage
	if isCA {
		ku |= x509.KeyUsageCertSign
	}
	if len(usages) == 0 {
		usages = append(usages, cmapi.DefaultKeyUsages()...)
	}
...
func GenerateCSR(crt *cmapi.Certificate) (*x509.CertificateRequest, error) {
...
func buildKeyUsagesExtensionsForCertificate(crt *cmapi.Certificate) ([]pkix.Extension, error) {
...
func GenerateTemplate(crt *cmapi.Certificate) (*x509.Certificate, error) {
	commonName := crt.Spec.CommonName
...
func SignatureAlgorithm(crt *cmapi.Certificate) (x509.PublicKeyAlgorithm, x509.SignatureAlgorithm, error) {
	var sigAlgo x509.SignatureAlgorithm
	var pubKeyAlgo x509.PublicKeyAlgorithm
	var specAlgorithm cmapi.PrivateKeyAlgorithm
