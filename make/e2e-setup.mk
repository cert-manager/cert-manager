...
IMAGE_kgateway_amd64 := ghcr.io/kgateway-dev/kgateway:v2.4.4@sha256:a041f8d6481b29dd1b6786e03e88e12d967727749dcd4678fb2b4bd588098d96
...
IMAGE_kgateway_arm64 := ghcr.io/kgateway-dev/kgateway:v2.4.4@sha256:ac4cf5630c3801e431c9b2f93c65b5c595ada584c127432e65948695bbc2352a
...
##
## @category Development
e2e-setup: e2e-setup-gatewayapi e2e-setup-certmanager e2e-setup-vault e2e-setup-bind e2e-setup-sampleexternalissuer e2e-setup-samplewebhook e2e-setup-pebble e2e-setup-ingressnginx e2e-setup-gwapi-provider
...
FEATURE_GATES ?= ExperimentalCertificateSigningRequestControllers=true,ExperimentalGatewayAPISupport=true,ListenerSets=true,ServerSideApply=true,LiteralCertificateSubject=true,UseCertificateRequestBasicConstraints=true,NameConstraints=true,OtherNames=true
...
feature_gates_controller := $(subst $(space),\$(comma),$(filter AllAlpha=% AllBeta=% ExperimentalCertificateSigningRequestControllers=% ExperimentalGatewayAPISupport=% ListenerSets=% ServerSideApply=% LiteralCertificateSubject=% UseCertificateRequestBasicConstraints=% NameConstraints=% SecretsFilteredCaching=% OtherNames=%, $(subst $(comma),$(space),$(FEATURE_GATES))))
...
e2e-setup-gwapi-provider: $(call image-tar,kgateway) load-$(call image-tar,kgateway) make/config/kgateway/gateway.yaml make/config/kgateway/gwconfig.yaml $(bin_dir)/scratch/kind-exists | $(NEEDS_HELM) $(NEEDS_KUBECTL)
	...
	@$(eval KGATEWAY_HELM_VERSION=v2.4.4)
