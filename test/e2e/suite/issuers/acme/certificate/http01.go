...
var _ = framework.CertManagerDescribe("ACME Certificate (HTTP01)", func() {
	...
	It("should obtain a signed certificate with a single CN from the ACME server", func() {
		...
		_, err := certClient.Create(util.NewCertManagerACMECertificate(certificateName, certificateSecretName, issuerName, v1alpha1.IssuerKind, 0, 0, acmeIngressClass, acmeIngressDomain))
		Expect(err).NotTo(HaveOccurred())
	...
	It("should obtain a signed certificate for a long domain using http01 validation", func() {
		...
		By("Creating a Certificate")
		_, err := certClient.Create(util.NewCertManagerACMECertificate(certificateName, certificateSecretName, issuerName, v1alpha1.IssuerKind, 0, 0, acmeIngressClass, fmt.Sprintf("%s.%s", cmutil.RandStringRunes(maxLengthOfDomainSegment), acmeIngressDomain)))
		Expect(err).NotTo(HaveOccurred())
		err = util.WaitCertificateIssuedValid(certClient, secretClient, certificateName, time.Minute*5)
		Expect(err).NotTo(HaveOccurred())
	})
	...
	It("should obtain a signed certificate with a CN and single subdomain as dns name from the ACME server", func() {
		...
		By("Creating a Certificate")
		_, err := certClient.Create(util.NewCertManagerACMECertificate(certificateName, certificateSecretName, issuerName, v1alpha1.IssuerKind, 0, 0, acmeIngressClass, acmeIngressDomain, fmt.Sprintf("%s.%s", cmutil.RandStringRunes(5), acmeIngressDomain)))
		Expect(err).NotTo(HaveOccurred())
		By("Verifying the Certificate is valid")
		err = util.WaitCertificateIssuedValid(certClient, secretClient, certificateName, time.Minute*5)
		Expect(err).NotTo(HaveOccurred())
	...
	It("should allow updating an existing certificate with a new dns name", func() {
		...
		By("Creating a Certificate")
		cert, err := certClient.Create(util.NewCertManagerACMECertificate(certificateName, certificateSecretName, issuerName, v1alpha1.IssuerKind, 0, 0, acmeIngressClass, acmeIngressDomain, fmt.Sprintf("%s.%s", cmutil.RandStringRunes(5), acmeIngressDomain)))
		Expect(err).NotTo(HaveOccurred())
		By("Verifying the Certificate is valid")
		err = util.WaitCertificateIssuedValid(certClient, secretClient, certificateName, time.Minute*5)
		Expect(err).NotTo(HaveOccurred())
	...
	It("should fail to obtain a certificate for an invalid ACME dns name", func() {
		...
		By("Creating a Certificate")
		_, err := f.CertManagerClientSet.CertmanagerV1alpha1().Certificates(f.Namespace.Name).Create(util.NewCertManagerACMECertificate(certificateName, certificateSecretName, issuerName, v1alpha1.IssuerKind, 0, 0, acmeIngressClass, "google.com"))
		Expect(err).NotTo(HaveOccurred())
