...
var _ = CloudDescribe("CertificateRequest with a properly configured Issuer", func() {
	f := framework.NewDefaultFramework("venafi-cloud-certificaterequest")
	h := f.Helper()

	var (
		issuer                 *cmapi.Issuer
		cloudAddon             = &vaddon.VenafiCloud{}
		certificateRequestName = "test-venafi-cloud-certificaterequest"
	)
	...
	It("should obtain a signed certificate for a single domain", func() {
		...
		dnsNames := []string{cmutil.RandStringRunes(10) + ".venafi-cloud-e2e.example"}
