...
func (v *VenafiTPP) Setup(ctx context.Context, cfg *config.Config, _ ...internal.AddonTransferableData) (internal.AddonTransferableData, error) {
	...
	if v.config.Addons.Venafi.TPP.URL == "" {
		return nil, errors.NewSkip(fmt.Errorf("CyberArk Control Plane Self-Hosted URL must be set"))
	}
	if v.config.Addons.Venafi.TPP.Zone == "" {
		return nil, errors.NewSkip(fmt.Errorf("CyberArk Control Plane Self-Hosted Zone must be set"))
	}

	if v.config.Addons.Venafi.TPP.AccessToken == "" {
		if v.config.Addons.Venafi.TPP.Username == "" {
			return nil, errors.NewSkip(fmt.Errorf("CyberArk Control Plane Self-Hosted requires either an access-token or username-password to be set: missing username"))
		}
		if v.config.Addons.Venafi.TPP.Password == "" {
			return nil, errors.NewSkip(fmt.Errorf("CyberArk Control Plane Self-Hosted requires either an access-token or username-password to be set: missing password"))
		}
	}
